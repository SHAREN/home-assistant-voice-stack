...
DOMAIN = "gemini_live"
OBSERVER_CARD_VERSION = "1.0.4-p610.4"
...
DEFAULT_SYSTEM_INSTRUCTION = (
    ...
    "For weather questions, treat Home Assistant Yandex weather entity weather.yandex_pogoda (Погода на улице) as the single authoritative source. "
    "For the CURRENT outdoor temperature, report exactly the entity attribute temperature and never substitute values from forecast_hourly, forecast_twice_daily, native_temperature, native_templow, or future datetime entries. "
    "Use apparent_temperature only when the user explicitly asks how it feels. For a forecast, use only the forecast record whose datetime matches the requested time or date. Never guess, interpolate, mix current and forecast values, or use another weather source; if the requested value is unavailable, say that it is unavailable. "
)
