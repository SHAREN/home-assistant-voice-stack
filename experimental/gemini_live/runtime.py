...
@dataclass(slots=True)
class StoredAudio:
    """Cached audio plus its privacy-safe phase trace."""
...
class TurnStore:
    ...
    def __init__(self) -> None:
        ...
        self._audio: deque[tuple[str, StoredAudio]] = deque(maxlen=100)
        self._streaming_audio: deque[
            tuple[str, TextStream, AudioStream, TurnTrace | None]
        ] = deque(maxlen=100)
        self._traces: deque[tuple[str, str, TurnTrace]] = deque(maxlen=100)
    ...
    def _discard_stale_voice_audio(self) -> None:
        ...
        for _text, stored in self._audio:
            if isinstance(stored.audio, AudioStream):
                stored.audio.finish()
        for _tts_message, _text_stream, audio_stream, _trace in self._streaming_audio:
            audio_stream.finish()
        self._audio = deque(
            (
                (text, stored)
                for text, stored in self._audio
                if not isinstance(stored.audio, AudioStream)
            ),
            maxlen=100,
        )
    ...
    def add_audio(
        ...
        if audio:
            self._audio.append((assistant_text, StoredAudio(audio, trace)))

    def take_audio(self, assistant_text: str) -> bytes | AudioStream | None:
        """Take the oldest audio response matching the TTS message."""
        stored = self.take_audio_entry(assistant_text)
        return stored.audio if stored is not None else None

    def take_audio_entry(self, assistant_text: str) -> StoredAudio | None:
        """Take cached audio with its optional observer trace."""
        for index, (text, stored) in enumerate(self._audio):
    ...
    def add_trace(
        ...
        """Queue one trace for the next matching conversation stage."""
        self._traces.append((conversation_id, user_text, trace))

    def take_trace(self, conversation_id: str, user_text: str) -> TurnTrace | None:
        """Consume the oldest trace matching the exact safe STT handoff."""
        for index, (stored_conversation_id, stored_text, trace) in enumerate(
            self._traces
        ):
            if stored_conversation_id == conversation_id and stored_text == user_text:
    ...
    def add_streaming_audio(
        ...
        self._streaming_audio.append((tts_message, text, audio, trace))
        _LOGGER.debug(
            "Queued streaming audio for TTS message_chars=%d transcript_chars=%d",
            len(tts_message),
            len(text.text),
        )

    def take_streaming_audio(self, initial_text: str) -> AudioStream | None:
        """Take streaming audio matching the exact TTS message or transcript."""
        stored = self.take_streaming_audio_entry(initial_text)
        return stored.audio if stored is not None else None

    def take_streaming_audio_entry(self, initial_text: str) -> StoredAudio | None:
        """Take matching streaming audio with its optional observer trace."""
        for index, (tts_message, text_stream, audio_stream, trace) in enumerate(
            self._streaming_audio
        ):
            transcript = text_stream.text
            message_matches = tts_message == initial_text
            transcript_matches = bool(transcript) and (
                transcript.startswith(initial_text) or initial_text.startswith(transcript)
            )
            if message_matches or transcript_matches:
                del self._streaming_audio[index]
                _LOGGER.debug(
                    "Matched streaming audio for TTS input chars=%d",
                    len(initial_text),
                )
                return StoredAudio(audio_stream, trace)
        _LOGGER.debug(
            "No streaming audio matched TTS input chars=%d",
            len(initial_text),
        )
        return None
