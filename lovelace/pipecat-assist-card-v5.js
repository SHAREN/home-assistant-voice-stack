const PIPECAT_ASSIST_CARD_VERSION = "0.1.75-audio-ice5";
const PIPECAT_ASSIST_ICE_SERVERS = [
...
const DEFAULT_AUDIO_BUFFER_MS = 1000;
...
class PipecatAssistCard extends HTMLElement {
  constructor() {
    super();
    this.installAudioDebug();
    this.stopOnPageExit = () => this.stop();
    this.audioVisibilityDebug = () => this.audioDebug("visibility", { visibility: document.visibilityState });
    this.backendReadyTimer = undefined;
  }
  ...
  installAudioDebug() {
    if (this._audioDebugInstalled) return;
    this._audioDebugInstalled = true;
    this._audioDebugSeq = 0;
    const initial = Boolean(this.audioBlocked);
    this._audioBlockedValue = initial;
    Object.defineProperty(this, "audioBlocked", {
      configurable: true,
      enumerable: true,
      get: () => this._audioBlockedValue,
      set: (value) => {
        const next = Boolean(value);
        const previous = this._audioBlockedValue;
        this._audioBlockedValue = next;
        if (previous !== next) {
          const stack = String(new Error().stack || "").split("\n").slice(2, 7).join(" | ");
          this.audioDebug("audioBlocked_change", { from: previous, to: next, stack });
        }
      },
    });
  }
  ...
  audioSnapshot() {
    ...
    const tracks = this.remoteStream?.getAudioTracks?.().map((track) => ({
      id: track.id,
    ...
    return {
      ...
      blocked: Boolean(this.audioBlocked),
      unlocked: Boolean(this.audioUnlocked),
      audioExists: Boolean(audio),
      audioConnected: Boolean(audio?.isConnected),
      paused: audio?.paused,
      muted: audio?.muted,
      volume: audio?.volume,
      ...
      srcKind: audio?.srcObject ? "media-stream" : (audio?.currentSrc ? "url" : "none"),
      remoteActive: Boolean(this.remoteStream?.active),
      tracks,
      peer: this.peer?.connectionState,
  ...
  audioDebug(event, extra = {}) {
    const data = {
      seq: ++this._audioDebugSeq,
      ts: new Date().toISOString(),
      event,
      ...this.audioSnapshot(),
      ...extra,
    };
    const message = JSON.stringify(data).slice(0, 7000);
    console.warn("[pipecat_audio_debug]", data);
    try {
      const result = this._hass?.callService?.("system_log", "write", {
        logger: "pipecat_audio_debug",
        level: "warning",
        message,
      });
      result?.catch?.(() => {});
    } catch {
      // Diagnostics must never affect the audio path.
    }
  }

  connectedCallback() {
    if (!window.isSecureContext && !["localhost", "127.0.0.1"].includes(window.location.hostname)) {
      const secureHost = this.config?.secure_host;
      if (secureHost) {
        const secureUrl = new URL(window.location.href);
        secureUrl.protocol = "https:";
        secureUrl.host = secureHost;
        window.location.replace(secureUrl.toString());
        return;
      }
      console.warn(
        "[pipecat] Browser microphone access requires HTTPS (or localhost). " +
        "Configure secure_host in the card or open Home Assistant over HTTPS."
      );
    }
    window.addEventListener("pagehide", this.stopOnPageExit);
    window.addEventListener("beforeunload", this.stopOnPageExit);
    document.addEventListener("visibilitychange", this.audioVisibilityDebug);
    this.ensureAudioElement();
    this.audioDebug("card_connected");
  }

  disconnectedCallback() {
    this.audioDebug("card_disconnected");
    window.removeEventListener("pagehide", this.stopOnPageExit);
    window.removeEventListener("beforeunload", this.stopOnPageExit);
    document.removeEventListener("visibilitychange", this.audioVisibilityDebug);
    this.stop();
    this.audio?.remove();
  ...
  setConfig(config) {
    ...
    this.audioUnlocked = Boolean(this.audioUnlocked);
    this.ensureAudioElement();
  ...
  async waitForIce(peerConnection, timeoutMs = 500) {
  ...
  ensureAudioElement() {
    if (!this.audio) {
      ...
      this.audio.style.cssText = "position:fixed;width:1px;height:1px;left:-9999px;top:-9999px;opacity:0;pointer-events:none";
      for (const eventName of [
        "loadstart", "loadedmetadata", "loadeddata", "canplay", "canplaythrough",
        "play", "playing", "pause", "waiting", "stalled", "suspend",
        "emptied", "ended", "abort", "error", "volumechange", "ratechange",
      ]) {
        this.audio.addEventListener(eventName, () => {
          this.audioDebug(`audio_event_${eventName}`, {
      ...
      this.audio.addEventListener("pause", () => {
        ...
        if (!running || !this.audioUnlocked || !this.remoteStream || this.endConversationStopping) return;
        window.setTimeout(() => {
          if (!this.audio || !this.remoteStream || this.endConversationStopping) return;
          this.audioDebug("pause_autoresume_attempt");
          this.audio.play().then(
            () => this.audioDebug("pause_autoresume_ok"),
            (err) => this.audioDebug("pause_autoresume_failed", { errorName: err?.name, errorMessage: err?.message }),
          );
        }, 0);
      });
      this.audioDebug("audio_element_created");
    }
    if (!this.audio.isConnected && document.body) {
      document.body.append(this.audio);
      this.audioDebug("audio_element_appended");
    }
    return this.audio;
  }
    ...
    try {
      if (this.remoteStream) {
        if (audio.srcObject !== this.remoteStream) {
          ...
          this.audioDebug("unlock_set_remote_stream");
        }
        audio.loop = false;
        ...
        audio.loop = true;
        this.audioDebug("unlock_set_silent_source");
      }
      await audio.play();
      this.audioDebug("unlock_play_ok");
    } catch (err) {
      this.audioDebug("unlock_play_failed", { errorName: err?.name, errorMessage: err?.message });
      this.audioUnlocked = false;
      this.audioBlocked = true;
      this.detail = this.t("audioBlocked");
      if (err?.name !== "NotAllowedError") console.warn("Pipecat audio unlock failed", err);
    }
  }

  resetAudioElement() {
    this.audioDebug("reset_audio_element", { stack: String(new Error().stack || "").split("\n").slice(2, 7).join(" | ") });
  ...
  stop() {
    ...
    if (this.backendReadyTimer) {
      clearTimeout(this.backendReadyTimer);
      this.backendReadyTimer = undefined;
    }
  ...
  async start() {
    ...
    try {
      ...
      this.cancelLocalSpeechResume();
      this.render();
      if (!this.audioUnlocked) this.resetAudioElement();
      await this.waitForAudioSessionRelease();
      const addonConfigPromise = this.proxyMode() ? Promise.resolve(null) : this.loadAddonConfig();
      this.audioDebug("get_user_media_begin");
      this.stream = await navigator.mediaDevices.getUserMedia({
        audio: { autoGainControl: true, echoCancellation: true, noiseSuppression: true },
        video: false,
      });
      this.audioDebug("get_user_media_ok", {
        inputTrack: this.stream.getAudioTracks()[0] ? {
          id: this.stream.getAudioTracks()[0].id,
          enabled: this.stream.getAudioTracks()[0].enabled,
          muted: this.stream.getAudioTracks()[0].muted,
      ...
      const peer = new RTCPeerConnection({
        iceServers: PIPECAT_ASSIST_ICE_SERVERS,
      ...
      this.channel.onopen = () => {
        ...
        this.audioDebug("backend_data_channel_open");
        this.render();
      ...
      peer.ontrack = (event) => {
        ...
        this.audioDebug("remote_track_received", {
          track: { id: event.track.id, enabled: event.track.enabled, muted: event.track.muted, readyState: event.track.readyState },
          streamCount: event.streams.length,
        });
        for (const eventName of ["mute", "unmute", "ended"]) {
          event.track.addEventListener(eventName, () => this.audioDebug(`remote_track_${eventName}`, {
            track: { id: event.track.id, enabled: event.track.enabled, muted: event.track.muted, readyState: event.track.readyState },
          }));
        }
        this.applyRemoteAudioBuffer(event.receiver);
        this.remoteStream = event.streams[0] || new MediaStream([event.track]);
        this.audioDebug("remote_stream_assigned");
        this.ensureVisualizer();
        this.attachAudio();
      };
      peer.onconnectionstatechange = () => {
        this.audioDebug("peer_connection_state", { connectionState: peer.connectionState, iceConnectionState: peer.iceConnectionState });
        if (peer.connectionState === "connected" && this.channel?.readyState !== "open") {
          this.state = "connecting";
          this.detail = "WebRTC connected; waiting for assistant backend";
          this.render();
        }
      ...
      const addonConfig = await addonConfigPromise;
      ...
      if (this.backendReadyTimer) clearTimeout(this.backendReadyTimer);
      this.backendReadyTimer = window.setTimeout(() => {
        this.backendReadyTimer = undefined;
        if (this.channel?.readyState !== "open" && !this.endConversationStopping) {
          this.audioDebug("backend_data_channel_timeout", { readyState: this.channel?.readyState });
          this.fail("Assistant backend did not start");
        }
      }, 15000);
  ...
  attachAudio() {
    if (!this.remoteStream) {
      this.audioDebug("attach_skipped_no_remote");
      return;
    }
    const audio = this.ensureAudioElement();
    this.audioDebug("attach_begin");
    audio.autoplay = true;
    audio.playsInline = true;
    audio.muted = false;
    audio.volume = 1;
    ...
    if (audio.srcObject !== this.remoteStream) {
      audio.srcObject = this.remoteStream;
      audio.removeAttribute("src");
      this.audioDebug("attach_srcobject_changed");
    }
    const playPromise = audio.play();
    if (playPromise?.then) {
      playPromise.then(
        () => this.audioDebug("attach_play_ok"),
        (err) => {
          this.audioDebug("attach_play_failed", { errorName: err?.name, errorMessage: err?.message });
  ...
  render() {
    ...
    if (audioButton) {
      audioButton.onclick = async () => {
        this.audioDebug("enable_audio_button_click");
        await this.unlockAudio();
        this.detail = this.t("connectedDetail");
        this.render();
      };
    }
    this.shadowRoot.querySelector(".main-button").onclick = async () => {
      this.audioDebug("main_button_click", { running });
      if (running) {
        this.stop();
        return;
      }
...
function collectPipecatAssistCards(root, cards = new Set(), seen = new Set()) {
  ...
  seen.add(root);
  if (root.localName === "pipecat-assist-card-v5") cards.add(root);
  if (!root.querySelectorAll) return cards;

  root.querySelectorAll("pipecat-assist-card-v5").forEach((card) => cards.add(card));
...
function refreshPipecatAssistCard(card) {
  ...
  card.installAudioDebug?.();
  card.audioBlocked = Boolean(card.audioBlocked);
  card.audioUnlocked = Boolean(card.audioUnlocked);
  card.ensureAudioElement?.();
...
const existingPipecatAssistCard = customElements.get("pipecat-assist-card-v5");
if (existingPipecatAssistCard) {
  patchPipecatAssistCard(existingPipecatAssistCard);
} else {
  customElements.define("pipecat-assist-card-v5", PipecatAssistCard);
}
installPipecatAssistCardRefresher();
refreshPipecatAssistCards();

window.customCards = Array.isArray(window.customCards) ? window.customCards : [];
const existingCardIndex = window.customCards.findIndex((card) => card.type === "pipecat-assist-card-v5");
if (existingCardIndex >= 0) window.customCards.splice(existingCardIndex, 1);
window.customCards.push({
  type: "pipecat-assist-card-v5",
  name: "Pipecat Assist",
