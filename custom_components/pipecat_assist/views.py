...
PROXY_CONFIG_CACHE_SECONDS = 300.0
PROXY_CONFIG_CACHE_KEY = "proxy_config_cache"
...
async def _load_addon_config(
    ...
    cache = domain_data.setdefault(PROXY_CONFIG_CACHE_KEY, {})
    cached = cache.get(entry.entry_id)
    now = time.monotonic()
    if cached and now - float(cached.get("loaded_at", 0.0)) < PROXY_CONFIG_CACHE_SECONDS:
    ...
    cache[entry.entry_id] = {"loaded_at": now, "data": dict(normalized)}
    return normalized
